...
func ParseAllSANExtension(value []byte) (dnsNames, emailAddresses []string, ipAddresses []net.IP, uris []*url.URL, directoryNames []pkix.Name, err error) {
	err = forEachSAN(value, func(tag int, data []byte) error {
		...
		case nameTypeDir:
			...
			var dirseq pkix.RDNSequence
			if _, err := asn1.Unmarshal(data, &dirseq); err != nil {
				return errors.New("x509: cannot parse directoryName")
			}

			var dirname pkix.Name
			dirname.FillFromRDNSequence(&dirseq)
