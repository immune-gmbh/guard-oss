...
func TestParseWinEventsTrustPointAgg(t *testing.T) {
	type eventLogAndPCRs struct {
		AllPCRs  map[string]map[string]api.Buffer `jsonapi:"attr,allpcrs" json:"allpcrs"`
		Firmware api.FirmwareProperties           `jsonapi:"attr,firmware" json:"firmware"`
	}

	buf, err := ioutil.ReadFile("../../test/IMN-SUPERMICRO-WBCL-Quote-AIKs.evidence.json")
	if !assert.NoError(t, err) {
		t.FailNow()
	}

	var ev eventLogAndPCRs
	err = json.Unmarshal(buf, &ev)
	if !assert.NoError(t, err) {
		t.FailNow()
	}

	els, err := UnpackTPM2EventLogZ(ev.Firmware.TPM2EventLogZ.Data)
	if err != nil {
		t.Fatalf("parsing event log: %v", err)
	}
	if !assert.Equal(t, 1, len(els), "wrong number of event logs found") {
		t.FailNow()
	}

	el, err := ParseEventLog(els[0])
	if err != nil {
		t.Fatalf("parsing event log: %v", err)
	}

	pcrs, err := ConvertPCRAPIBufferMapToEventLogPCRs(ev.AllPCRs["11"], crypto.SHA256)
	if err != nil {
		t.Fatalf("converting PCRs: %v", err)
	}

	_, err = el.Verify(pcrs)
	if err != nil {
		t.Fatalf("validating event log: %v", err)
	}

	// get all events with SHA256 and not the ones verified against PCRs above
	// b/c the event we look for is not extended to PCRs
	winEvents, err := ParseWinEvents(el.Events(HashSHA256))
	if err != nil {
		t.Fatalf("ParseWinEvents() failed: %v", err)
	}

	_, ok := winEvents.TrustPointQuote["bla"]
	if !ok {
		t.Fatal("required key 'bla' not found")
	}

	// since we're here, check all quotes
	for key, quote := range winEvents.TrustPointQuote {
		assert.NotEmpty(t, key, "key name empty")
		assert.Equal(t, 256, len(quote.QuoteSignature), "quote signature has wrong length")

		_, err := tpm2.DecodeAttestationData(quote.Quote)
		if !assert.NoErrorf(t, err, "tpm2.DecodeAttestationData() failed for key: %s", key) {
			continue
		}

		pubBlob, ok := ev.Firmware.PCPQuoteKeys[key]
		if assert.Truef(t, ok, "quote signed with following key has") {
			pub, err := windows.ExtractTPMTPublic(pubBlob)
			if !assert.NoErrorf(t, err, "windows.ExtractTPMTPublic() failed for key: %s", key) {
				continue
			}

			cryptoPub, err := pub.Key()
			if !assert.NoErrorf(t, err, "failed to convert TPM key to crypto key: %s", key) {
				continue
			}

			valid := verifyQuoteSignature(quote.Quote, quote.QuoteSignature, cryptoPub)
			assert.Truef(t, valid, "quote signature invalid for key: %s", key)
		}
	}
}

func TestParseWinEventsCounters(t *testing.T) {
	type eventLogAndPCRs struct {
		AllPCRs  map[string]map[string]api.Buffer `jsonapi:"attr,allpcrs" json:"allpcrs"`
		Firmware api.FirmwareProperties           `jsonapi:"attr,firmware" json:"firmware"`
	}

	buf, err := ioutil.ReadFile("../../test/IMN-DELL-WBCL-BootCtrs.evidence.json")
	if !assert.NoError(t, err) {
		t.FailNow()
	}
	...
	assert.Equal(t, uint64(85), winEvents.BootCount, "wrong boot counter value decoded")
	assert.Equal(t, uint64(1536), winEvents.EventCount, "wrong event counter value decoded")
	assert.Equal(t, uint64(21037057), winEvents.EventCounterId, "wrong event counter ID decoded")
}
