...
const ROLE_OPTIONS = { all: 'All', admin: 'Admin', owner: 'Owner', user: 'User' } as const;

function AdminOrganisationUsers(): JSX.Element {
  const router = useRouter();
  const organisationId = router.query.id as string;
  const { data, isLoading } = useOrganisation({ id: organisationId, 'include[]': 'users' });
  ...
  return (
    <AdminLayout className="space-y-4">
      <Headline>User Management - {data.name}</Headline>
      <div className="flex justify-between">
        ...
        <ImmuneLink
          theme="MAIN"
          href={{
            pathname: NextJsRoutes.adminInvitePath,
