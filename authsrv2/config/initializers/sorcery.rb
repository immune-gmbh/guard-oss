...
Rails.application.config.sorcery.configure do |config|
  ...
  if !Rails.env.test?
    ...
    #
    config.twitter.key = Settings.authentication.twitter.api_key
    config.twitter.secret = ENV.fetch "AUTHSRV_TWITTER_API_SECRET" do
      Rails.application.credentials.twitter[:api_secret]
    end
    config.twitter.callback_url = Settings.authentication.twitter.callback_url
    config.twitter.user_info_mapping = {:name => "screen_name", :email => "email"}
    config.twitter.user_info_path="/1.1/account/verify_credentials.json?include_entities=false&skip_status=true&include_email=true"
  end
  #
  # config.facebook.key = ""
  ...
  #
  if !Rails.env.test?
    config.github.key = Settings.authentication.github.api_key
    config.github.secret = ENV.fetch "AUTHSRV_GITHUB_API_SECRET" do
      Rails.application.credentials.github[:api_secret]
    end
    config.github.callback_url = Settings.authentication.github_callback_url
  ...
  #
  if !Rails.env.test?
    config.google.key = Settings.authentication.google.api_key
    config.google.secret = ENV.fetch "AUTHSRV_GOOGLE_API_SECRET" do
      Rails.application.credentials.google[:api_secret]
    end
    config.google.callback_url = Settings.authentication.google.callback_url
  ...
  config.user_config do |user|
    ...
    # Default: `5 * 60`
    #
    # user.reset_password_time_between_emails = 15
    ...
    # Default: `:user_id`
    #
    # user.authentications_user_id_attribute_name = :user_public_id
